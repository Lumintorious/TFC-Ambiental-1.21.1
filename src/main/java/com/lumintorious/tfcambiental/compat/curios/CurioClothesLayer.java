    ...
    private void renderArmorPiece(ItemStack itemstack, PoseStack poseStack, MultiBufferSource bufferSource, T entity, ArmorItem.Type armorType, int light, A playerModel) {
        if (itemstack.getItem() instanceof ClothesItem clothesItem) {
            if (clothesItem.getType().equals(armorType)) {
                ...
                Model model = ClientHooks.getArmorModel(entity, itemstack, armorType.getSlot(), playerModel);
                this.renderModel(poseStack, bufferSource, light, itemstack.hasFoil(), model, this.getArmorTexture(entity, itemstack, armorType, null, playerModel));
            }
        }
    }

    public ResourceLocation getArmorTexture(net.minecraft.world.entity.Entity entity, ItemStack stack, ArmorItem.Type slot, @Nullable String type, A playerModel) {
        ClothesItem item = (ClothesItem) stack.getItem();
        ResourceLocation texture = ResourceLocation.withDefaultNamespace("hi"); // item.getMaterial().getName() /);
...
//        ClientHooks.getArmorModel(entity, stack, s1, slot.getSlot()); // TODO jdk remove?
        ResourceLocation resourcelocation = ARMOR_LOCATION_CACHE.get(s1);

        ResourceLocation.read(s1).ifSuccess(resourceLocation -> {
    ...
    public void renderModel(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, boolean hasFoil, Model model, ResourceLocation armorResource) {
        if (armorResource != null) {
            VertexConsumer vertexconsumer = ItemRenderer.getArmorFoilBuffer(bufferSource, RenderType.armorCutoutNoCull(armorResource), hasFoil);
